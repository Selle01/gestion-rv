...
$(document).on('click', '.editeSecretary', async function (e) {
    ...
    await axios.get(url).then(({ data }) => {
        ...
        $("#service_id option[value=" + data.genre + "]").attr('selected', true);
        ...
        $("#service_id option[value=" + data.role_id + "]").attr('selected', true);
...
const table = $('#tableSecretary').DataTable({
    ...
    columns: [
        ...
        },
        {
            title: "action",
            data: function (data) {
                const route_edite = SECRETARY_URL + '/' + data.id + '/edit';
                const route_delete = SECRETARY_URL + '/' + data.id + '/delete';
