...
class MedecinDao extends CommonDao
{
    ...
    {
        $sql = "SELECT * FROM  medecin m JOIN user u ON m.user_id =u.id WHERE u.status=TRUE ORDER BY m.id DESC";
    ...
    {
        $query = $this->pdo->prepare("SELECT * FROM medecin m JOIN user u ON m.user_id =u.id WHERE status=TRUE AND u.id = :id");
        $query->execute(['id' => $id]);
